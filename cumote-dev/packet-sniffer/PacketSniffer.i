...
void m644_init_usart(void);
    ...
    interrupt [21] void     handle_rxc      (void);
    interrupt [23] void     handle_txc      (void);
    ...
    void m644_init_usart(void)  {
        m644_baud_divisor = 1;           
        busy_flag = 0;
        (*(unsigned char *) 0xc0) = 0b01000010;  // double comm speed
        (*(unsigned char *) 0xc1) = 0b11000000;  // enable rx and tx interrupts
        (*(unsigned char *) 0xc2) = 0b00000110;  // asynchronous, no parity, one stop bit, 8 data bits
        (*(unsigned char *) 0xc5) = 0;
        (*(unsigned char *) 0xc4) = 0;  // 0.5 MBps baud rate    
        ...
        busy_flag = 0;
        (*(unsigned char *) 0xc0) = 0b01000000;  
        (*(unsigned char *) 0xc1) = 0b11000000;
        (*(unsigned char *) 0xc2) = 0b00000110;  // asynchronous, no parity, one stop bit, 8 data bits
        (*(unsigned char *) 0xc5) = 0;
        (*(unsigned char *) 0xc4) = 25;         //9600 baud
    }
        ...
        if (usart_msg.i < usart_msg.length)  {
            ...
            (*(unsigned char *) 0xc6) = usart_msg.string[usart_msg.i++];  // send first element
        }
    }
...
//    }                                                       
        ...
        (*(unsigned char *) 0xc1) |= 0b00010000;
    }
        inline void m644_disable_rx(void)  {
        (*(unsigned char *) 0xc1) &= 0b11101111;
    }
        inline void m644_enable_tx(void)  {
        (*(unsigned char *) 0xc1) |= 0b00001000;
    }
        inline void m644_disable_tx(void)  {
        (*(unsigned char *) 0xc1) &= 0b11110111;
    }      
        inline void m644_enable_trx(void)  {
        (*(unsigned char *) 0xc1) |= 0b00011000;
    }
        inline void m644_disable_trx(void)  {
        (*(unsigned char *) 0xc1) &= 0b11100111;
    }                 
        interrupt [21] void handle_rxc(void)  {        
        unsigned char junk;  // used to throw away received data if buffer is full
        if (usart_rx_buffer.num_elements < 10)  {
            usart_rx_buffer.buff[usart_rx_buffer.tail++] = (*(unsigned char *) 0xc6);
            usart_rx_buffer.num_elements++;
            if (usart_rx_buffer.tail == 10)
                usart_rx_buffer.tail = 0;
        }             
        else {
            junk = (*(unsigned char *) 0xc6);
        }
    }
        ...
        if (usart_msg.i == usart_msg.length)  {
            ...
            (*(unsigned char *) 0xc6) = usart_msg.string[usart_msg.i++];  // send next byte
        }
    }                  
...
interrupt [14] void handle_tim1(void) {
    if (ms_counter > 0)
        ms_counter--;
}
    ...
    ack_msg[3] = 0xdd;
        ...
        if (is_buffer_empty() == 0)  {
            ...
            if (rx_state == 1)  {
                ...
                if (my_rx_data == 0xca)  {
                    HAL_set_state(0x06);  // turn rx on if it's off
                    sniffer_state = 2     ;         
                    usart_rx_ack();
                }
                               else if (my_rx_data == 0x9d  )  {
                    HAL_set_state(0x08);  // turn rx off if it is on
                    sniffer_state = 1;
                    usart_rx_ack();
                }
                            else if (my_rx_data == 0xba  )  {
                    HAL_set_state(0x06);  // turn rx on
                    sniffer_state = 4;
                    usart_rx_ack();
                    ms_counter = 0;
                }
                            else if (my_rx_data == 0xab)  {
                    rx_state = 1;
                }
            }
        }
            ...
            if (COM_IRQ_pending == 1)  {  // radio has info for the application
                ...
                else if (COM_IRQ_status == 0x08       )  {  // either we missed the RX_START IRQ or this is just happening at the end of a received frame that we've already started downloading.
                }
            }                        
        }
                else if (sniffer_state == 4)  {
            if (ms_counter == 0)  {
